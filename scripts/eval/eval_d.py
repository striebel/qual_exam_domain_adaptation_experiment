...
def main():
    ...
    assert os.path.isdir(RSLT_DIR)
    
    _file_path = os.path.join(RSLT_DIR, 'domain_to_is_done.json')
    assert os.path.isfile(_file_path)
    _file = open(_file_path, 'r')
    _file_str = _file.read()
    _file.close()
    domain_to_is_done = json.loads(_file_str)
    ...
    n_pred_domains_ready = sum(
        [1 if is_done else 0 for domain, is_done in domain_to_is_done.items()]
    )
    print(f'n_pred_domains_ready : {n_pred_domains_ready}')
    input('press enter to continue')

    n_pred_files_ready = n_pred_domains_ready * 21 * 10
    i = 1
    ...
    for domain in [
        ...
        assert os.path.isdir(pred_domain_dir), \
            'pred_domain_dir: '+pred_domain_dir
    ...
    n_pred_proportions_ready = (1 + n_pred_domains_ready) * 21
    j = 1
    ...
    for domain in [
        ...
        rslt_domain_dir = os.path.join(RSLT_DIR, domain)
        if 'all' == domain and not os.path.isdir(rslt_domain_dir):
            os.mkdir(rslt_domain_dir)
        assert os.path.isdir(rslt_domain_dir), \
            'rslt_domain_dir : '+rslt_domain_dir
        
        for proportion in [
            '000', '005', '010', '015', '020', '025', '030', '035', '040', '045',
            '050', '055', '060', '065', '070', '075', '080', '085', '090', '095', '100'
        ]:
            print(f'processing {j: >3} / {n_pred_proportions_ready}')
            j += 1
            
            rslt_proportion_dir = os.path.join(rslt_domain_dir, proportion)
            if 'all' == domain and not os.path.isdir(rslt_proportion_dir):
                os.mkdir(rslt_proportion_dir)
            assert os.path.isdir(rslt_proportion_dir)

...
class MachampUniversalReader(DatasetReader):
    ...
    def _read(self, file_path: str) -> Iterable[Instance]:
        ...
        for dataset in self.datasets:
            ...
            assert 'domain'     in self.datasets[dataset]
            assert 'proportion' in self.datasets[dataset]
            assert 'fold'       in self.datasets[dataset]
    ...
    def read_sequence(self, dataset, path, is_train, max_sents, domain=None, proportion=None, fold=None):
        """
        ...
        in zip(list_of_conllu_objs, seqs2data(path, self.do_lowercase)):
            ...
            _instance = self.text_to_instance(sent_tasks, full_data, col_idxs, is_train, task2type, dataset, dataset_embeds, conllu_obj, domain, proportion, fold)
